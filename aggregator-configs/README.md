...
|---|---|---|
| Vector | `tests/aggregator/roundtrip/` — real generator-produced events pushed through the exact committed `transform.json`'s VRL, via the official `timberio/vector:latest-alpine` image, asserting every declared field extracts with the right value | Yes, but **not** part of `npm test` — needs Docker. Run it explicitly: `tests/aggregator/roundtrip/run.sh` |
